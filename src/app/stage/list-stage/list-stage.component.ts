...
export class ListStageComponent implements OnInit {
  // Define searchQuery property to hold the search query
  loggedInUsername: string | null;
  userId: number; // Declare a variable to hold the user id
  ...
  //searchQuery: any;
  constructor(public crudApi: StageService, public toastr: ToastrService,public artService: StageService,
  ...
  ngOnInit() {
   ...
   // this.userId = this.userService.getUserId(); // Fetch the user id from AuthService
  ...
  searchByDate(event: Event): void {
    ...
    if (selectedDate.trim() === '') {
      ...
      // Otherwise, filter items based on date incident
      this.filteredItems = this.crudApi.list.filter(item =>
        item.dateIncident === selectedDate
      );
    }
...
selectData(item: Stage) {
  ...
  onFileSelected(event) {
    ...
    // Convert selected image to base64 data URL
  ...
  const document = this.getDocument(currentDate); // Pass currentDate to your document generation function
  alert("mrigl");
    ...
    return {
      table: {
        ...
        body: [
         ...
         ...items.map(ed => {
            return [ed.id,ed.nom_entreprise, ed.titreStage, ed.description];
          })
        ]
      }
    };
